...
public class Sample1 {
	...
	public static void main(String[] args) {
		...
		// .charAt(int index)
		// 문자열에서 index 위치에 해당하는 문자를 반환, 반환타임(char)
		char c = s.charAt(2);
		...
		boolean b2= s.equals("문자열 관련 메서드");
		System.out.println("s.equals(\"문자열 관련메서드\") ->" +b1);
		System.out.println("s.equals(\"String 관련메서드\") ->" +b2);
		...
		// 문자열과 anotherString의 문자열을 비교하여 사전순으로 기준 문자열이 앞이면
		// 음수, 같으면 0, 뒤면 양수값을 반
		String comp = new String("ABCD");
		int diff1 = comp.compareTo("BCDE");
		int diff2 = comp.compareTo("AACD");
		System.out.println("commp.compareTo(\"BCDE\")->"+diff1);
		System.out.println("commp.compareTo(\"AACD\")->"+diff2);
		...
		// .equals와 동일한 기능 단, 대소문자 구분을 하지 않는다. 
		...
		// . trim();
		// 문자열에서 공백을 제거하면 반환, 단 문장ㄹ의 앞/뒤의 공백만 제거한다. 반환타입(String)
		// 사용자 입력에서 사용자가 불필요한 고백을 입력하는 경우 제거할 수 있다.
		...
		String formatStr = String.format("%d %.2f %s",date[0], date[1], date[2]);
		System.out.println(formatStr);
		
		// String.format(String format, Object ...args)
		// 클래스 메서드로 format 문자열 형식에 해당하는 문자열을 반환
		// String format str = String.format("%s-%s-%s", date[0], date[1], date[2]);
		// 
	}
